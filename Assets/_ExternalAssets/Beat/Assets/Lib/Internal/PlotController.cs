...
public class PlotController : MonoBehaviour {
	...
	[SerializeField]float displayMultiplier = 0.1f;
	...
	public virtual void updatePlot(List<SpectralFluxInfo> pointInfo, int curIndex = -1) {
		...
		for (int i = windowStart; i < windowEnd; i++) {
			...
			Transform fluxPoint = plotPoints [plotIndex].Find ("FluxPoint");//Вторичный звук
			Transform threshPoint = plotPoints [plotIndex].Find ("ThreshPoint"); // Фоновый шум
			Transform peakPoint = plotPoints [plotIndex].Find ("PeakPoint"); // Основной звук
