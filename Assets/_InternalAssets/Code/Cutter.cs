...
public class Cutter : MonoBehaviour {
    ...
    public static void Cut(GameObject originalGameObject, Vector3 contactPoint, Vector3 cutNormal) {
        ...
        GeneratedMesh leftMesh = new GeneratedMesh(originalMesh);
        GeneratedMesh rightMesh = new GeneratedMesh(originalMesh);
        ...
        //Getting and destroying all original cotlidors to prevent having multiple colliders
        // of different kinds on one object

        var originalCols = originalGameObject.GetComponents<Collider>();
        foreach (var item in originalCols)
        {
            Destroy(item);
        }
        ...
        right.transform.position = originalGameObject.transform.position + (Vector3.up * 0.05f);
        ...
        rightRigidbody.AddRelativeForce(-cutPlane.normal * 250);*/
    ...
    public static void SeparateMeshes(GeneratedMesh leftMesh, GeneratedMesh rightMesh, Plane plane, List<Vector3> addedVerticles)
    {
            ...
            for (int j = 0; j < subMeshIndices.Length; j+=3)
            {
                ...
                switch (triangleALeftSide)
                {
                    ...
                    case false when !triangleBLeftSide && !triangleCLeftSide:
                        ...
                        break;
                    default:
                        CutTriangle(plane, currentTriangle, triangleALeftSide, triangleBLeftSide, triangleCLeftSide, leftMesh, rightMesh, addedVerticles);
                        break;
                }
            }
        }
    ...
    private static MeshTriangle GetTriangle(int _triangleIndexA, int _triangleIndexB, int _triangleIndexC, int _submeshIndex)
    {
        ...
        return new MeshTriangle(verticeToAdd, normalsToAdd, uvsToAdd, _submeshIndex);
    }
    ...
    public static void FillCut(List<Vector3> _addedVertices, Plane plane, GeneratedMesh _leftMesh, GeneratedMesh _rightMesh)
    {
        ...
        for (int i = 0; i < _addedVertices.Count; i++)
        {
            if (!vertices.Contains(_addedVertices[i]))
            {
                ...
                Fill(polygon, plane, _leftMesh, _rightMesh);
            }
        }
    }
...
public class GeneratedMesh
{
    ...
    public GeneratedMesh(Mesh originalMesh)
    {
        submeshTriangles = new List<int>[originalMesh.subMeshCount + 1]; // +1 for the new submesh created by the cut
    ...
    public void AddTriangle(MeshTriangle triangle)
    {
        ...
        //uvs.AddRange(triangle.uv);

        submeshTriangles[triangle.SubmeshIndex].Add(startIndex);
        submeshTriangles[triangle.SubmeshIndex].Add(startIndex + 1);
        submeshTriangles[triangle.SubmeshIndex].Add(startIndex + 2);
    }
